...
void* multithread(void* pix)
{
    ...
    for( int px=*st; px<width; px+=nthreads)
    {
...
main( int argc, char **argv )
{
    ...
    pthread_t *threadnum = malloc(nthreads * sizeof(pthread_t));
    ...
    printf( "# Rendering scene with %d spheres and %d lights\n",
            scene.sphere_count,
            scene.light_count );
    int* startpt = (int*) malloc(nthreads * sizeof(int));
    for (int i = 0; i<nthreads;i++)
        startpt[i]=i;
    for (int i = 0; i < nthreads; i++)
    {
        pthread_create(threadnum + i, NULL, multithread, (startpt+i));
    }
    for( int i=0; i < nthreads; i++)
